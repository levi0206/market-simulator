...
class MarketGenerator:
    ...
    def _load_data(self):
        try:
            ...
            print("shape of downloaded data:",self.data.shape)
        except:
            raise RuntimeError(f"Could not download data for {self.ticker} from {self.start} to {self.end}.")

        self.windows = []

        # Sample some data from self.data
        # and apply lead-lag transformation
    ...
    def _build_dataset(self):
        ...
        # MinMaxScaler: 
        # Translates each feature individually such that it is in the given range 
        ...
        print("self.logsig shape:{}".format(self.logsigs.shape))
        self.conditions = logsig[:-1]
        print("self.conditions shape:{}".format(self.conditions.shape))

    def train(self, n_epochs=10000):
        self.generator = CVAE(data=self.logsigs, data_cond=self.conditions, latent_dim=8, alpha=0.003)
